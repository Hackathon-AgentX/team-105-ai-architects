...
def page_attendance():
    ...
    teams    = bot.get_teams()
    marks    = bot.get_all_marks()
    return render_template("attendance.html", students=students, total=total,
                           present=present, absent=total-present, teams=teams, marks=marks)
...
def api_save_marks():
    ...
    if team_id is None or marks is None:
        return jsonify({"status": "error", "message": "team_id and marks required."}), 400
    try:
        marks = float(marks)
    except ValueError:
        return jsonify({"status": "error", "message": "marks must be a number."}), 400
    if marks < 0 or marks > 100:
        return jsonify({"status": "error", "message": "marks must be between 0 and 100."}), 400
    bot.save_marks(team_id, marks, remarks)
    return jsonify({"status": "ok", "team_id": team_id, "marks": marks})
